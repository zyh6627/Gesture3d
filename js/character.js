...
/**
 * Create an outlined mesh group.
 * Uses inverted-hull technique: a slightly scaled black back-face mesh behind the main mesh.
 */
function outlinedMesh(geometry, material, outlineThickness = 0.02) {
  const group = new THREE.Group();

  // Outline: black, slightly larger, back-face rendering
  const outlineMat = new THREE.MeshBasicMaterial({
    color: C.black,
    side: THREE.BackSide,
  });
  const outline = new THREE.Mesh(geometry.clone(), outlineMat);
  const scale = 1 + outlineThickness;
...
function buildHead() {
  ...
  const { group: skull } = outlinedMesh(headGeo, headMat, 0.025);
  headGroup.add(skull);
...
function buildBody() {
  ...
  const { group: bodyOutline } = outlinedMesh(bodyGeo, bodyMat, 0.025);
  ...
  for (let side = -1; side <= 1; side += 2) {
    ...
    const { group: sleeveGrp } = outlinedMesh(sleeveGeo, sleeveMat, 0.02);
    sleeveGrp.position.set(side * 0.8, -0.05, 0.15);
    sleeveGrp.rotation.z = side * 0.35;
    sleeveGrp.rotation.x = 0.25;
    bodyGroup.add(sleeveGrp);
  }
...
function buildHand() {
  ...
  const { group: palmGrp } = outlinedMesh(palmGeo, palmMat, 0.02);
  handGroup.add(palmGrp);

  // Fingers — 4 small cylinders
  for (let i = 0; i < 4; i++) {
    const fingerGeo = new THREE.CapsuleGeometry(0.045, 0.25, 8, 8);
    const { group: fingerGrp } = outlinedMesh(fingerGeo, palmMat, 0.02);
    fingerGrp.position.set(-0.1 + i * 0.07, 0.18, 0.04);
    fingerGrp.rotation.x = -0.15;
    handGroup.add(fingerGrp);
  }
...
function buildCup() {
  ...
  const { group: cupOutline } = outlinedMesh(cupGeo, cupMat, 0.025);
